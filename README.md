...
![Interface of the SDF-FFT Generator](./doc/images/SDFFFTInterface.png)
...
* `busy: Bool` - status register, active in the state `sFlushData`
...
* `sFlush` - in this state data flushing is active. Once when `pktEndDetect` (indicator that previous fft window has been processed) is asserted, FSM transits to `sIdle` state.
