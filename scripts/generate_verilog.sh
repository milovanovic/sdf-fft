...
generate_verilog_radix2 () {
  ...
  do
    ...
    do
      ...
      cd $RDIR && sbt "runMain dspblocks.sdffft.SDFFFTApp $fft_dir_name $width $fft_size $bitreverse 2 ${separate_verilog}"
...
generate_verilog_radix22 () {
  ...
  do
    ...
    do
      ...
      cd $RDIR && sbt "runMain dspblocks.sdffft.SDFFFTApp $fft_dir_name  $width $fft_size $bitreverse 2^2 ${separate_verilog}"
      if [ -d $fft_dir_name  ]; then mv $RDIR/mem.conf $fft_dir_name/fft_mem.conf;fi
    done
  done
}
