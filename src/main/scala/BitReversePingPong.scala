...
class BitReversePingPong[T <: Data: Real](val params: BitReversePingPongParams[T]) extends Module {
  ...
  val readyWrite = state_next =/= StateFSM.sReadOnly
  val cntWritten = CounterWithReset(io.in.valid && readyWrite, size, rstCntWritten) // and state idle
  //val cntRead = CounterWithReset(io.out.fire(), size, rstCntRead)
  val validRead = state === StateFSM.sReadOnly || state === StateFSM.sReadWrite // and state idle
 ...
 // when (io.in.ready && readyWrite && ~writePing) {
  ...
  io.out.valid := RegNext(state === StateFSM.sReadOnly || state === StateFSM.sReadWrite) // state_prev
