...
class SDFChainRadix2[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  val cntrStages = Wire(Vec(numStages, UInt(log2Up(params.numPoints).W)))
  ...
  switch (state) {
    ...
    is (sFlush) {
      when (pktEndDetect) { // frame is processed or window is processed
        state_next := sIdle
      }
    }
  }
  ...
  val rstCntVal = if (params.decimType == DIFDecimType) cntrStages(numStages.U - regNumStages) else cntrStages(regNumStages - 1.U)
  ...
  val newOutCnt = cntr_wires(regNumStages-1.U)
  val newOutEn = enableVector(regNumStages-1.U)
  
  // some kind of optimization occurs when numPoints is 2 - not clear what exactly happens!
  if (params.runTime == true) {
    when (newOutCnt === (numPoints - 2.U) && newOutEn === true.B) {
   ...
   .elsewhen (state_next === sIdle) {
    ...
    when (cntr_wires(regNumStages-1) === (params.numPoints - 2).U && newOutEn === true.B) {
      initialOutDone := true.B
    }
    .elsewhen (state_next === sIdle) {
      initialOutDone := false.B
  ...
  val hasWorthwhileOutput = newOutEn && initialOutDone
  ...
  sdf_stages.zipWithIndex.map { case (e, idx) => {
      ...
      if (params.decimType == DIFDecimType) {
        cntrStages(idx) := (cntr_wires(index) - (cumulativeDelaysWires(idx) - cumulativeDelayWire)) (delayLog2s(idx),0)
      }
      else {
        cntrStages(idx) := (cntr_wires(0))(delayLog2s(idx),0)
      }
  ...
  outQueue.io.enq.valid := ShiftRegister(hasWorthwhileOutput, outputLatency, resetData = false.B, en = true.B) && state =/= sIdle
  outQueue.io.deq.ready := io.out.ready
  
  io.in.ready := io.out.ready && (state =/= sFlush)
  // there is no need to divide it by N if scale logic is included
  ...
  io.out.bits := fft_out_bits //outQueue.io.deq.bits
  io.out.valid := outQueue.io.deq.valid
  val cntValidData = CounterWithReset(io.out.valid && io.out.ready, params.numPoints, pktEndDetect) //works good for both DIT and DIF algorithm and run-time configurability
  pktEndDetect := cntValidData._1 === (numPoints - 1.U)
  io.lastOut := (state === sFlush) && pktEndDetect
  if (params.overflowReg) {
    io.overflow.get := overflowReg
  }
  io.busy := state === sFlush // maybe redundant!
}
