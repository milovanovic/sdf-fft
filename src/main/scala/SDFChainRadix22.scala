...
class SDFChainRadix22[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  switch (state) {
    ...
    is (sFlush) {
      when (pktEndDetect) { //frame is processed
        state_next := sIdle
      }
    }
  }
  ...
  // take care of the run time configurability also
  sdf_stages.map(_.io).zipWithIndex.foldLeft(input_data) {
    case (stg_in, (stg_io, index)) => {
      ...
      if (params.decimType == DIFDecimType)
        ...
        input := stg_in
      if (params.decimType == DIFDecimType) {
         cntrStages(index) := (cntr_wires(index.U - (numStages.U - regNumStages)) - (cumulativeDelaysWires(index) - cumulativeDelayWire)) (delayLog2s(index),0)
      }
      else {
         cntrStages(index) := (cntr_wires(0))(delayLog2s(index),0)
      }
      if (params.decimType == DIFDecimType) {
      ...
      if (isMulTw && !doNothing) {
        ...
        if (params.decimType == DIFDecimType) {
          ...
          twiddles(index) := ShiftRegister(uniqueTwiddleTable(twiddleIdxTable(address)), params.numAddPipes, en = true.B)
        }
        else {
          cntrStages(index) := (cntr_wires(0))(delayLog2s(index),0)
...
//           DspContext.alter(DspContext.current.copy(trimType = NoTrim, overflowType = Grow, complexUse4Muls = params.use4Muls)) { stg_in context_* twiddles(index) }
  ...
  val cntValidData = CounterWithReset(io.out.valid && io.out.ready, params.numPoints, pktEndDetect)
