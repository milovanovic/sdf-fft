...
class SDFChainRadix22RunTime[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  val outCnt = cntr_wires(regNumStages-1.U) //make Mux in verilog code
  val outEn =  enableVector(regNumStages - 1)
  
 // Note: error for numPoints = 2 run vs compile time
  if (params.runTime == true) {
    when (outCnt === (numPoints - 2.U) && outEn === true.B) {
    ...
    when (cntr_wires(regNumStages-1) === (params.numPoints - 2).U && outEn === true.B) {
      initialOutDone := true.B
    }
    .elsewhen (state_next === sIdle) {
      initialOutDone := false.B
  ...
  val hasWorthwhileOutput = outEn && initialOutDone
  
  val output = if (params.decimType == DIFDecimType) ShiftRegister(outputWires.last, complexMulLatency, en = true.B) else outputWires(regNumStages-1)
...
object SDFRadix22AppRunTime extends App
{
  val params = FFTParams.fixed(
    ...
    numMulPipes = 0,
    runTime = false,
    expandLogic = Array.fill(log2Up(16))(1),
