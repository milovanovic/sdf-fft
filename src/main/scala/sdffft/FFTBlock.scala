...
trait AXI4FFTFixedStandaloneBlock extends AXI4FFTBlock[FixedPoint] {
...
abstract class FFTBlock[T <: Data: Real: BinaryRepresentation, D, U, E, O, B <: Data](
  ...
  println(((outputWidth + 8 - 1) / 8).toString)
  println(outputWidth)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    if (params.zeroPadderParams.isDefined) {
      val packetSizeStart = RegInit(params.numPoints.U(log2Up(params.numPoints).W))
      val numberOfPackets = RegInit(2.U(log2Up(params.zeroPadderParams.get.numberOfPackets)))
      fft.io.packetSizeStart.get := packetSizeStart
      fft.io.numberOfPackets.get := numberOfPackets
    ...
    if (params.windowFunc != WindowFunctionTypes.None()) {
      val enWind = RegInit(false.B)
      fft.io.enWind.get := enWind
      commonFields = commonFields :+ RegField(
        1,
        enWind,
    ...
    println(outputWidth / 2 - fft.io.out.bits.real.getWidth)
    val offset = 0.U((outputWidth / 2 - fft.io.out.bits.real.getWidth).W)
    // this should work appropriately
    out.bits.data := Cat(Cat(fft.io.out.bits.real.asUInt, offset), Cat(fft.io.out.bits.imag.asUInt, offset))
...
object FFTDspBlockFixedTL extends App {
...
object FFTDspBlockFixedAXI4 extends App {
  val paramsFFT = FFTParams.fixed(
    dataWidth = 12,
    twiddleWidth = 16,
    numPoints = 256,
  ...
  val lazyDut = LazyModule(
    new AXI4FFTBlock(paramsFFT, AddressSet(baseAddress + 0x100, 0xff), _beatBytes = 4, configInterface = false)
      with AXI4FFTFixedStandaloneBlock
  )
...
object FFTDspBlockFixedAXI4WithConfig extends App {
  val paramsFFT = FFTParams.fixed(
    dataWidth = 16,
    twiddleWidth = 16,
    numPoints = 1024,
  ...
  val baseAddress = 0x500 // just to check if verilog code is successfully generated or not
  implicit val p: Parameters = Parameters.empty
  val lazyDut = LazyModule(
