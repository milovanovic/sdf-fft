...
abstract class MultipleFFTsBlockFixed[T <: Data: Real: BinaryRepresentation, D, U, E, O, B <: Data](
  params:          FFTParams[T],
  beatBytes:       Int,
  ...
  val outputWidth = params.protoIQ.getWidth + 2 * params.expandLogic.count(_ == 1)
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val packetSizeStart =
      if (params.zeroPadderParams.isDefined) Some(RegInit(params.numPoints.U(log2Up(params.numPoints).W))) else None
    val numberOfPackets =
      if (params.zeroPadderParams.isDefined) Some(RegInit(2.U(log2Up(params.zeroPadderParams.get.numberOfPackets))))
      else None
    ...
    val enWind = if (params.windowFunc != WindowFunctionTypes.None()) Some(RegInit(false.B)) else None
    ...
    if (params.zeroPadderParams.isDefined) {
      commonFields = commonFields :+ RegField(
        log2Up(params.numPoints),
        packetSizeStart.get,
        RegFieldDesc(name = "packetSizeStart", desc = "defines number of samples inside initial input packet")
      )
      commonFields = commonFields :+ RegField(
        log2Up(params.zeroPadderParams.get.numberOfPackets),
        numberOfPackets.get,
    ...
    if (params.windowFunc != WindowFunctionTypes.None()) {
      commonFields = commonFields :+ RegField(
        1,
        enWind.get,
        RegFieldDesc(name = "enable signal", desc = "defines enable signal for windowing")
      )
    }
    ...
    for ((in, inIdx) <- ins.zipWithIndex) {
      ...
      if (params.runTime) {
        if (configInterface) {
          when(configMode.get === true.B) {
            when(configNode.get.in.head._1.fire) {
              configReg.get := configNode.get.in.head._1.bits.data.asUInt
            }
            configNode.get.in.head._1.ready := ~busy
            fft.io.fftSize.get := configReg.get
          }.otherwise {
            fft.io.fftSize.get := fftSize.get
          }
          configNode.get.in.head._1.bits.last := false.B //DontCare*/
        } else {
          fft.io.fftSize.get := fftSize.get
        }
      }
      ...
      if (params.zeroPadderParams.isDefined) {
        fft.io.packetSizeStart.get := packetSizeStart.get
        fft.io.numberOfPackets.get := numberOfPackets.get
      }
      if (params.windowFunc != WindowFunctionTypes.None()) {
        fft.io.enWind.get := enWind.get
      }
      ...
      val offset = 0.U((outputWidth / 2 - fft.io.out.bits.real.getWidth).W)
      outs(inIdx).bits.data := Cat(
        Cat(fft.io.out.bits.real.asUInt, offset),
        Cat(fft.io.out.bits.imag.asUInt, offset)
      ) //fft.io.out.bits.asUInt
...
class AXI4MultipleFFTsBlock[T <: Data: Real: BinaryRepresentation](
  ...
  implicit p: Parameters)
    extends MultipleFFTsBlockFixed[
      T,
...
class TLMultipleFFTsBlockFixed[T <: Data: Real: BinaryRepresentation](
  ...
  implicit p: Parameters)
    extends MultipleFFTsBlockFixed[T, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle](
      params,
      beatBytes
    )
    with TLDspBlock
    with TLHasCSR {
  val devname = "TLMultipleFFTsBlockFixed"
...
object MultipleFFTsDspBlockFixedTL extends App {
...
object MultipleFFTsDspBlockFixedAXI4 extends App {
  val paramsMultipleFFTs = FFTParams.fixed(
    dataWidth = 12,
    twiddleWidth = 16,
    numPoints = 256,
    ...
    numAddPipes = 1,
    numMulPipes = 1,
    expandLogic = Array.fill(log2Up(256))(1),
...
// For the pynq!
object MultipleFFTsDspBlockAXI4FixedForPynq extends App {
  val paramsMultipleFFTs = FFTParams.fixed(
    dataWidth = 12,
    twiddleWidth = 16,
    numPoints = 512,
