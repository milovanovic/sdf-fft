...
object WindowFunctionTypes {
  case class Hamming(dataWidth_tmp: Int = 16, alpha: Double = 0.54, beta: Double = 0.46, isPeriodic: Boolean = true)
      extends WindowFunctionType {
    val dataWidth = dataWidth_tmp
    override def toString =
      "Hamming window (" + alpha + ", " + beta + ")" // add info whether it is symmetric or periodic
  }
...
object WindowFunctions {
  ...
  def hammingWindow(length: Int, alpha: Double = 0.54, beta: Double = 0.46, isPeriodic: Boolean = true): Seq[Double] = {
    if (length == 1) Seq(1.0)
      ...
      (0 until (length + 1)).map((count: Int) => alpha - beta * cos(2 * Pi * count.toDouble / length)).take(length)
  ...
  def triangularWindow(length: Int, isPeriodic: Boolean = true): Seq[Double] = {
    if (length == 1) Seq(1.0)
      ...
      (0 until (length + 1)).map((count: Int) => 1 - abs((count.toDouble - length / 2.0) / (length / 2.0))).take(length)
  ...
  def blackmanWindow(
    ...
    if (length == 1) Seq(1.0)
      ...
      (0 until (length + 1)).map((count: Int) =>
        a0 - a1 * cos(2 * Pi * count.toDouble / length) + a2 * cos(4 * Pi * count.toDouble / length)
      )
    else
      (0 until length).map((count: Int) =>
        a0 - a1 * cos(2 * Pi * count.toDouble / (length - 1)) + a2 * cos(4 * Pi * count.toDouble / (length - 1))
      )
  }
