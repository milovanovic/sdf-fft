...
class WindowingBlock[T <: Data: Real: BinaryRepresentation](
...
//     }
    ...
    // dontTouch(r_addr)
    val winCoeff = if (params.constWindow) windowROM(address_rom) else windowMem(r_addr_reg)
    ...
    val inComplex =
      if (params.constWindow) in.bits.data.asTypeOf(params.protoIQ) else RegNext(in.bits.data.asTypeOf(params.protoIQ))
...
object WindowingBlockApp extends App {
  ...
  (new ChiselStage).execute(
    Array("--target-dir", "verilog/WindowingBlock"),
