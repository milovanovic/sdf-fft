...
/**
* This test should run simple FFT testers and show functionality of the core without detailed analysis.
  ...
  it should f"compute and plot radix 2^2 fft of the size 1024, no growing logic and FixedPoint data type " in {//ignore {
    ...
    dsptools.Driver.execute(
      ...
      new FixedPointSDFFFTTester(c) {
        ...
        f.saveas(s"test_run_dir/FFT_sin_1024_radix22.pdf")
      }
    } should be (true)
  }
  
  it should f"compute and plot radix-2^2 fft of the size 1024, no growing logic and FixedPoint data type (test signal with noise)" in {//in {
    val paramsFixed = FFTParams.fixed(
      numPoints = 1024,
      dataWidth = 16,
      numAddPipes = 1,
    ...
    dsptools.Driver.execute(
      ...
      new FixedPointSDFFFTTester(c) {
        ...
        p_fft.xlabel = "Frequency Bin"
        p_fft.ylabel = "Magnitude"
        f.saveas(s"test_run_dir/FFT_sin_1024_radix22_noisy.pdf")
      }
    } should be (true)
  }
  
  it should f"compute and plot radix-2^2 fft/ifft of the size 512, no growing logic and FixedPoint data type " in {
    val paramsFixed = FFTParams.fixed(
    numPoints = 512,
    dataWidth = 16,
    numAddPipes = 1,
    ...
    dsptools.Driver.execute(
      ...
      new FixedPointSDFFFTTester(c) {
        ...
        val p_sin = f.subplot(0)
        p_sin.legend_=(true)
        val xaxis = (0 until testSignal.length).map(e => e.toDouble).toSeq.toArray //PROVERI OVO!
        val testSignalReal = testSignal.map(e => e.real)
        ...
        f.saveas(s"test_run_dir/FFT_IFFT_sin_512_radix22_FixedPoint.pdf")
      }
    } should be (true)
 }
  
   it should f"compute and plot radix-2^2 fft/ifft of the size 512 and no growing logic and DspReal data type " in {
  ...
  //  dataWidth = 16,//this parameter is not relevant for DSPReal testing
    ...
    dsptools.Driver.execute(
      ...
      new DspRealSDFFFTTester(c) {
          ...
          f.saveas(s"test_run_dir/FFT_IFFT_sin_512_radix22_DspReal.pdf")
        }
    } should be (true)
  }
}
