...
class Radix22SQNRSpec extends AnyFlatSpec with Matchers {
  ...
  */
  ...
  p1 += plot(xaxis, sqnr_results16_scale, name = "16-bit round")
  p1 += plot(xaxis, sqnr_results24_scale, name = "24-bit round")
  p1 += plot(xaxis, sqnr_results16_grow, name = "16-bit grow")
  p1 += plot(xaxis, sqnr_results24_grow, name = "24-bit grow")
  ...
  p1.ylabel = "SQNR [dB]"
  f1.saveas(s"test_run_dir/sqnrDataWidthRadix22.pdf")
  ...
  p2.title_= (s"Signal-to-Quantization-Noise Ratio (SQNR)" + "\n" + " radix 2^2 - DIF")
  
  p2.xlabel = "Number of FFT stages (base 2 logarithm of the FFT window size)"
  p2.ylabel = "SQNR [dB]"
  f2.saveas(s"test_run_dir/sqnrRoundingModesRadix22.pdf")
