...
import dsptools.numbers._//include DspReal
...
trait SDFFFTTester[T <: chisel3.Data] extends HasTesterUtil[SDFFFT[T]]{ this: PeekPokeTester[SDFFFT[T]] =>
  ...
  /**
   * Tests fft frame with and without flushing data
   ...
   // step(5)
    ...
    while (cntValidOut < inp.size) {
      if (peek(c.io.out.valid) == true && cntValidOut < inp.size) {
        fixTolLSBs.withValue(tolLSBs) { expect(c.io.out.bits, out(output1.length)/scalingFactor) }
        output1 = output1 :+ peek(c.io.out.bits) // append new data
        cntValidOut +=1
      }
      step(1)
    }
    //poke(c.io.lastIn, 0) // can be reset after one clock cycle
    step(2)
    ...
    while (output2.length < fftSize) {
      if (input2.hasNext && peek(c.io.in.ready)) {
...
class FixedSDFFFTTester {
  ...
  def fixedTester(params: FFTParams[FixedPoint], testSignal: Seq[Complex], tolLSBs: Int = 2): Boolean = {
    dsptools.Driver.execute(() => new SDFFFT(params), Array("-tbn", "verilator")){ c => new FixedPointSDFFFTTester(c) { 
      if (params.useBitReverse) this.testBitReversal(tolLSBs, testSignal, params) else this.testFFT(tolLSBs, testSignal, params) } }
  }
