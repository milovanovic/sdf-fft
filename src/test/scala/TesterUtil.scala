...
trait HasTesterUtil[T <: Module] extends DspTester[T] {
   ...
   */
  def plot_fft (chiselFFT: Seq[Complex], scalaFFT: Seq[Complex]): Unit = {
    import breeze.linalg._
    ...
    val chiselPlot = chiselFFT.map(c => Seq(c.abs, plotMin).max).toSeq //chiselFFT.map(c => 20 * math.log10(Seq(c.abs, plotMin).max)).toSeq
    val scalaPlot = scalaFFT.map(c => Seq(c.abs, plotMin).max).toSeq //scalaFFT.map(c => 20 * math.log10(Seq(c.abs, plotMin).max)).toSeq
