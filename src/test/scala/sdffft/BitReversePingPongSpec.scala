...
class BitReversePingPongTester[T <: chisel3.Data](c: BitReversePingPong[T])
  ...
  def testReadyOut(inp: Seq[Complex], tol: Double = 0.001): Unit = {

    val input = if (c.params.bitReverseDir) bitrevorder_data(inp) else inp
    val bitRevOrderedData = bitrevorder_data(input)
    val output =
      if (c.params.fftShiftEnable)
        bitRevOrderedData
          .slice(inp.size / 2, inp.size)
          .concat(bitRevOrderedData.slice(0, inp.size / 2))
      else bitRevOrderedData
    ...
    while (cntValidOut < output.length) {
      if (cntValidOut == (output.length - 1) && peek(c.io.out.valid) == 1) {
        expect(c.io.lastOut, 1)
        //fixTolLSBs.withValue(tol) { expect(c.io.out.bits, output(cntValidOut)) }
        assert(abs(output(cntValidOut) - peek(c.io.out.bits)) <= tol, "Mismatch!!!")
        cntValidOut += 1
      } else {
    ...
    while (cntValidIn < input.length) {
      ...
      if (peek(c.io.out.valid) == 1 && peek(c.io.out.ready) == 1) {
        //fixTolLSBs.withValue(tol) { expect(c.io.out.bits, output(cntValidOut)) }
        assert(abs(output(cntValidOut) - peek(c.io.out.bits)) <= tol, "Mismatch!!!")
        cntValidOut += 1
      }
  ...
  def testStream(inp: Seq[Complex], tol: Double = 0.001): Unit = {
  ...
  def testAdjustableSize(inp: Seq[Complex], tol: Double = 0.001): Unit = {
    ...
    while (idx < 1) {
      ...
      for (size <- sizeCollShuffle) {
        ...
        val output =
          if (c.params.fftShiftEnable)
            bitRevOrderedData
              .slice(size / 2, inp.size)
              .concat(bitRevOrderedData.slice(0, size / 2))
          else bitRevOrderedData
        ...
        while (input1.hasNext && peek(c.io.in.ready) == 1) {
          poke(c.io.in.bits, input1.next())
          step(1)
        }
        while (input2.hasNext && peek(c.io.in.ready) == 1) {
          ...
          if (peek(c.io.out.valid) == 1) {
            ...
            assert(abs(output(cntValid) - peek(c.io.out.bits)) <= tol, "Mismatch!!!")
            if (cntValid == (size - 1))
              cntValid = 0
            else
              cntValid += 1
        ...
        while (input3.hasNext && peek(c.io.in.ready) == 1) {
...
//  }
//}
class BitReversePingPongSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  for (fftShiftEnable <- Seq(true, false)) {
    it should f"test bitreverse ping-pong without adjustableSize and bitReverseDir = true and fftShiftEnable equal to $fftShiftEnable" in { // DIF
      ...
      test(new BitReversePingPong(paramsPingPong))
        .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation))
        .runPeekPoke(new BitReversePingPongTester(_) {
          testStream(testSignal)
          //testAdjustableSize(inp)
        })
    }
  }

  for (fftShiftEnable <- Seq(true, false)) {
    it should f"test bitreverse ping-pong without adjustableSize and bitReverseDir = false and fftShiftEnable equal to $fftShiftEnable" in { // DIT
  ...
  for (fftShiftEnable <- Seq(true, false)) {
    it should f"test bitreverse ping-pong with adjustableSize and bitReverseDir = true and fftShiftEnable equal to $fftShiftEnable" in {
      // input is in bitreverse order
      // output is in natural
      val paramsPingPong = BitReversePingPongParams.fixed(
        dataWidth = 16,
      ...
      test(new BitReversePingPong(paramsPingPong))
        .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation))
        .runPeekPoke(new BitReversePingPongTester(_) {
          testAdjustableSize(testSignal)
        })
    }
  }
  for (fftShiftEnable <- Seq(true, false)) {
    it should f"test bitreverse ping-pong with adjustableSize and bitReverseDir = true and single port SRAMs and fftShiftEnable equal to $fftShiftEnable" in {
      // input is in bitreverse order
      // output is in natural
      val paramsPingPong = BitReversePingPongParams.fixed(
        dataWidth = 16,
  ...
  for (fftShiftEnable <- Seq(true, false)) {
    it should f"test bitreverse ping-pong when ready out is changing  and fftShiftEnable equal to $fftShiftEnable" in {
      val paramsPingPong = BitReversePingPongParams.fixed(
        dataWidth = 16,
        binPoint = 14,
        pingPongSize = pingPongSize,
