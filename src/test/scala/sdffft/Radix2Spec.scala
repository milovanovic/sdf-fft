 ...
 */
class Radix2Spec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  // check DIT variant as well
  for (i <- Seq(32)) {
    for (decType <- Seq(DIFDecimType)) {
      for (numPipes <- Seq(Seq(1, 1))) {
        for (isBitReverse <- Seq(true)) {
          val addPipes = numPipes(0)
          val mulPipes = numPipes(1)
          it should f"compute radix 2 $decType FFT, size $i with no growing logic and numAddPipes = $addPipes and numMulPipes = $mulPipes, bitReverse = $isBitReverse - test last signal" in {
            val testSignal = getRealTone(i, (1.0 / i))
            val paramsFixed = FFTParams.fixed(
              ...
              windowFunc = WindowFunctionTypes.None(),
            ...
            test(new SDFFFT(paramsFixed))
              ...
              .runPeekPoke(new SDFFFTTester(_) {
                val result = testRandomizedFFT(0.001, testSignal, paramsFixed)
              })
          }
        }
      }
  ...
  // check DIT variant as well
  for (i <- Seq(8, 16, 32, 64)) {
    for (decType <- Seq(DIFDecimType)) {
      for (numPipes <- Seq(Seq(0, 0), Seq(1, 0), Seq(1, 1), Seq(3, 2))) {
        for (isBitReverse <- Seq(true, false)) {
          val addPipes = numPipes(0)
          val mulPipes = numPipes(1)
          it should f"compute radix 2 $decType FFT, size $i with no growing logic and numAddPipes = $addPipes and numMulPipes = $mulPipes, bitReverse = $isBitReverse and windowing" in {
            val testSignal = getRealTone(i, (1.0 / i))
            val paramsFixed = FFTParams.fixed(
              numPoints = i,
              numMulPipes = mulPipes,
            ...
            test(new SDFFFT(paramsFixed))
              .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation, LogLevelAnnotation(LogLevel.Info)))
              .runPeekPoke(new SDFFFTTester(_) {
                val result = simpleTestFFT(0.001, testSignal, paramsFixed)
              })
          }
        }
      }
      ...
      for (numPipes <- Seq(Seq(0, 0), Seq(1, 0), Seq(1, 1), Seq(3, 2))) {
        ...
        it should f"compute radix 2 $decType FFT, size $i with no growing logic and numAddPipes = $addPipes and numMulPipes = $mulPipes" in {
          ...
          test(new SDFFFT(paramsFixed))
            .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation, LogLevelAnnotation(LogLevel.Info)))
            .runPeekPoke(new SDFFFTTester(_) {
              val result = simpleTestFFT(0.000001, testSignal, paramsFixed)
            })
        }
      }
    }
    ...
    for (decType <- Seq(DIFDecimType, DITDecimType)) {
      it should f"compute radix 2 $decType FFT, size $i with no growing logic and 4 multiplier structure" in {
        ...
        test(new SDFFFT(paramsFixed))
          .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation, LogLevelAnnotation(LogLevel.Info)))
          .runPeekPoke(new SDFFFTTester(_) {
            val result = testFFT(0.0001, testSignal, paramsFixed)
          })
      }
    }
  }
  ...
  for (i <- Seq(32, 128)) {
    for (decType <- Seq(DIFDecimType, DITDecimType)) {
      for (singlePortSRAM <- Seq(true, false)) {
        it should f"compute radix 2 $decType FFT, size $i, singlePortSRAM = $singlePortSRAM with no growing logic and included bit-reversal stage" in {
          val testSignal = getRealTone(i, (1.0 / i))
          val paramsFixed = FFTParams.fixed(
            numPoints = i,
            numAddPipes = 1,
