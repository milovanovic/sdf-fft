...
//
//     val chiselPlot =
//       chiselFFT.map(c => if (dB) 20 * math.log10(Seq(c.abs, plotMin).max) else Seq(c.abs, plotMin).max).toSeq
//     val scalaPlot =
//       scalaFFT.map(c => if (dB) 20 * math.log10(Seq(c.abs, plotMin).max) else Seq(c.abs, plotMin).max).toSeq
//
